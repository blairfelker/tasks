...
export function editOption(
    ...
    const questionMaker = function (
        ...
        return {
            ...question,
            options: optionMap(question, newOption, targetOptionIndex)
        };
    };

    const optionMap = function (
        question: Question,
        newOption: string,
        targetOptionIndex: number
    ): string[] {
        return question.options.map((option: string): string =>
            option === question.options.at(targetOptionIndex)
                ? newOption
                : option
        );
    };
    ...
    return questions.map(
        (question: Question): Question =>
            question.id !== targetId
                ? question
                : questionMaker(question, newOption, targetOptionIndex)
    );
}
